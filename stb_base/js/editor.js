...
// ── Node names for randomize ──
let NODE_NAMES = window.NODE_NAMES_DATA || {};
...
function renderNodes() {
  ...
  for (const node of tree.nodes) {
    ...
    const MMO_HEX = {red:'#c0392b',green:'#27ae60',pink:'#8e44ad',blue:'#2980b9',yellow:'#f39c12'};
    const nodeColor = MMO_HEX[node.mmoColor] || '#222736';
    const nodeBorder = node.id===selectedId ? 'var(--accent)' : (node.mmoColor ? adjustColor(nodeColor) : 'var(--border2)');
    el.innerHTML = `
...
function renderColorPicker(node) {
  const picker = document.getElementById('mmo-color-picker');
  const label  = document.getElementById('mmo-color-selected');
  if (!picker) return;
  ...
  const current = node.mmoColor || 'red';
  if (label) label.textContent = `เลือก: ${colors[current] || current}`;

  picker.querySelectorAll('.mmo-color-btn').forEach(btn => {
    btn.classList.toggle('active', btn.dataset.color === current);
    btn.onclick = () => {
      node.mmoColor = btn.dataset.color;
      // อัพ label
      if (label) label.textContent = `เลือก: ${colors[btn.dataset.color] || btn.dataset.color}`;
      picker.querySelectorAll('.mmo-color-btn').forEach(b => b.classList.toggle('active', b.dataset.color === btn.dataset.color));
      renderNodes();
      markDirty();
    };
  });
...
async function exportMMOCore() {
  if (!window.exportToMMOCore) {
    SwalDark.fire({icon:'error', title:'ไม่พบ exporter', text:'mmocore_exporter.js โหลดไม่ได้', confirmButtonText:'ตกลง'});
    return;
  }

  const { value: opts } = await SwalDark.fire({
    title: '⚔ Export MMOCore YAML',
    html: `
      <div style="text-align:left">
        <div style="margin-bottom:10px">
          <label style="font-size:12px;color:var(--text2);display:block;margin-bottom:4px">ชื่อ Tree (tree id)</label>
          <input id="mmo-name" class="swal2-input" value="${tree.meta?.name||'my_tree'}"
            style="margin:0;font-size:13px" placeholder="my_tree">
        </div>
        <div style="margin-bottom:10px">
          <label style="font-size:12px;color:var(--text2);display:block;margin-bottom:4px">Max Points</label>
          <input id="mmo-pts" type="number" class="swal2-input" value="100"
            style="margin:0;font-size:13px" min="1" max="9999">
        </div>
        <div style="font-size:11px;color:var(--text3);margin-top:8px">
          ⚠ stat จะถูกแปลงเป็น FLAT type ทั้งหมดตาม MMOCore format
        </div>
      </div>
    `,
    showCancelButton: true,
    ...
    cancelButtonText: 'ยกเลิก',
    preConfirm: () => ({
      treeName:  document.getElementById('mmo-name').value.trim() || 'my_tree',
      maxPoints: parseInt(document.getElementById('mmo-pts').value) || 100,
    })
  });
  ...
  let yaml;
  try {
    yaml = window.exportToMMOCore(tree, opts);
  } catch(e) {
    SwalDark.fire({icon:'error', title:'Export ผิดพลาด', text: e.message, confirmButtonText:'ตกลง'});
    return;
  }
  ...
  a.download = (opts.treeName || 'tree') + '.yml';
  a.click();

  SwalDark.fire({
    icon:'success',
    title:'Export MMOCore สำเร็จ!',
    html: `ดาวน์โหลด <b>${opts.treeName}.yml</b> แล้ว<br>
      <span style="font-size:11px;color:var(--text3)">นำไปวางใน plugins/MMOCore/skilltrees/ ได้เลย</span>`,
    toast: false,
...
function showRandomizeNameMenu(node) {
  const cats = Object.keys(NODE_NAMES);
  if (cats.length === 0) {
    SwalDark.fire({icon:'warning', title:'ไม่พบไฟล์ชื่อ', text:'กรุณาใส่ชื่อใน data/node_names.json ก่อน', confirmButtonText:'ตกลง'});
    return;
  }
  SwalDark.fire({
    title: '🎲 สุ่มชื่อ Node',
      ...
      <div style="display:flex;flex-direction:column;gap:8px;margin-top:8px">
        ${cats.map(cat => `
          <button class="swal2-confirm"
            onclick="Swal.close();window._doRandName('${cat}')"
            style="width:100%;padding:8px;border-radius:6px;cursor:pointer;font-size:13px;text-transform:capitalize">
            ${cat}
          </button>`).join('')}
        <button class="swal2-confirm"
          onclick="Swal.close();window._doRandName('__all__')"
          style="width:100%;padding:8px;border-radius:6px;cursor:pointer;font-size:13px;background:var(--gold2);border-color:var(--gold)">
          ✨ สุ่มจากทุกหมวด
        </button>
      </div>`,
    showConfirmButton: false,
    ...
    cancelButtonText: 'ยกเลิก',
    didOpen: () => {
      window._doRandName = (cat) => {
        let pool = [];
        if (cat === '__all__') {
          pool = Object.values(NODE_NAMES).flat();
        } else {
          pool = NODE_NAMES[cat] || [];
        }
        ...
        renderNodes();
        markDirty();
        SwalDark.fire({icon:'success', title:`"${name}"`, toast:true, position:'top-end', showConfirmButton:false, timer:1500});
      };
    }
  });
}
...
function bindEvents() {
  ...
  document.getElementById('btn-randomize-name').addEventListener('click', () => {
    if (!selectedId) return;
    const node = TreeModel.getNode(tree, selectedId);
    if (node) showRandomizeNameMenu(node);
  });

  document.getElementById('btn-new').addEventListener('click', newTree);
  document.getElementById('btn-save').addEventListener('click', doSave);
  document.getElementById('btn-export').addEventListener('click', exportJSON);
  document.getElementById('btn-export-mmo').addEventListener('click', exportMMOCore);
